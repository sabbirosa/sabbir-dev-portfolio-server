...
class AuthController {
    async login(req, res) {
        try {
            ...
            const user = await User_mongoose_1.UserModel.findOne({ email: loginData.email }).select("+password");
            if (!user) {
            ...
            logger_1.logger.logAuth("LOGIN_SUCCESS", user._id.toString(), user.email);
            const response = {
                success: true,
    ...
    async getProfile(req, res) {
        try {
            ...
            const fullUser = await User_mongoose_1.UserModel.findById(user.id);
            if (!fullUser) {
...
//# sourceMappingURL=AuthController.mongoose.js.map