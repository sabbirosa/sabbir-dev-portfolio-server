...
exports.seedAll = exports.seedProjects = exports.seedBlogs = exports.seedAdminUser = void 0;
...
const seedAdminUser = async () => {
    try {
        const existingAdmin = await User_mongoose_1.UserModel.findOne({ email: environment_1.env.ADMIN_EMAIL });
        if (existingAdmin) {
            logger_1.logger.info("Admin user already exists", { email: environment_1.env.ADMIN_EMAIL });
            return;
        }
        const adminUser = await User_mongoose_1.UserModel.create({
            email: environment_1.env.ADMIN_EMAIL,
            password: environment_1.env.ADMIN_PASSWORD,
            role: "admin",
        });
        ...
        logger_1.logger.error("Error seeding admin user", { error: error.message });
        throw error;
    }
};
...
const seedBlogs = async () => {
    try {
        const blogCount = await Blog_1.BlogModel.countDocuments();
        if (blogCount > 0) {
            logger_1.logger.info(`${blogCount} blogs already exist, skipping seed`);
            return;
        }
        const sampleBlogs = [
            {
                title: "Building a Modern Portfolio with React and Tailwind CSS",
                description: "Learn how to create a stunning developer portfolio using React, Tailwind CSS, and modern web development practices. This comprehensive guide covers everything from setup to deployment.",
...
A well-crafted portfolio can open doors to new opportunities. Take time to polish it and keep it updated with your latest projects.`,
            ...
            },
            {
                ...
                description: "Explore different state management solutions in React, from useState and useContext to Redux and Zustand. Understand when and how to use each approach effectively.",
...
Understanding these tools will help you build more maintainable React applications.`,
            ...
            },
            {
                ...
                description: "Discover advanced techniques for optimizing Next.js applications, including image optimization, code splitting, and server-side rendering strategies for better user experience.",
...
Optimization is an ongoing process. Regularly monitor and improve your application's performance.`,
        ...
        await Blog_1.BlogModel.insertMany(sampleBlogs);
        logger_1.logger.info(`Seeded ${sampleBlogs.length} sample blogs`);
    }
    catch (error) {
        logger_1.logger.error("Error seeding blogs", { error: error.message });
        throw error;
    }
};
...
const seedProjects = async () => {
    try {
        const projectCount = await Project_1.ProjectModel.countDocuments();
        if (projectCount > 0) {
            logger_1.logger.info(`${projectCount} projects already exist, skipping seed`);
            return;
        }
        const sampleProjects = [
            {
                title: "BUCC Portal",
                description: "Full-stack portal for BRAC University Computer Club (1500+ members, 33+ admins). Streamlined recruitment & events, RFID attendance via student IDs, and role-based dashboards.",
            ...
            {
                title: "Uddyog",
                description: "Platform for social development events. SPA with JWT auth + Firebase login, event creation/filtering, reviews & participation tracking, theme toggle, and newsletter support.",
            ...
            {
                title: "Cohabit",
                description: "Roommate-matching SPA by location, budget, and lifestyle. Secure auth (email + Google OAuth), protected routes, theme toggle, Swiper gallery, and mobile-first UI.",
            ...
            {
                title: "Project R",
                description: "Donation management system with MySQL-backed REST API. Role-based dashboards, authentication, donation tracking, reporting, and easy local/remote deployment.",
            ...
            {
                title: "Civix UI",
                description: "Accessible component library for e-government platforms (thesis). Standardizes UX for citizens with limited digital literacy; supports clearer flows and form guidance.",
        ...
        await Project_1.ProjectModel.insertMany(sampleProjects);
        logger_1.logger.info(`Seeded ${sampleProjects.length} sample projects`);
    }
    catch (error) {
        logger_1.logger.error("Error seeding projects", { error: error.message });
        throw error;
    }
};
...
const seedAll = async () => {
    try {
        ...
        await (0, exports.seedBlogs)();
        await (0, exports.seedProjects)();
        logger_1.logger.info("Database seeding completed successfully");
    }
    catch (error) {
        logger_1.logger.error("Database seeding failed", { error: error.message });
        throw error;
    }
};
exports.seedAll = seedAll;
//# sourceMappingURL=seed.mongoose.js.map